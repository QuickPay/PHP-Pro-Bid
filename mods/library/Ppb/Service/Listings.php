            ...
            foreach ($params as $key => $value) {
                ...
                if (preg_match('#^(custom_field_)#', $key) && !empty($value)) {
                    ...
                    if (in_array($fieldId, $customFields)) {
                        ...
                        $customField = $customFieldsService->findBy('id', $fieldId);

                        if ($customField['is_range']) {
                            $rangeFrom = isset($value[Range::RANGE_FROM]) ? doubleval($value[Range::RANGE_FROM]) : null;
                            $rangeTo = isset($value[Range::RANGE_TO]) ? doubleval($value[Range::RANGE_TO]) : null;

                            $customFieldsSelect = array();
                            if ($rangeFrom > 0) {
                                $customFieldsSelect[] = "{$alias}.value >= '{$rangeFrom}'";
                            }
                            if ($rangeTo > 0) {
                                $customFieldsSelect[] = "{$alias}.value <= '{$rangeTo}'";
                            }

                            $customFieldsSelect = implode(' AND ', $customFieldsSelect);
                        }
                        ## -- ./ADD -- [ MOD:- ADVANCED CUSTOM FIELDS ]
